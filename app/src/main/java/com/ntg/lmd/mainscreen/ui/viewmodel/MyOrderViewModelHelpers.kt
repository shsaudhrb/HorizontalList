...
class OrdersPaginator(
    ...
    suspend fun loadFirst(uid: String?): PageBundle {
        ...
        endReached = false
        val res = getMyOrders(
            page = 1, limit = pageSize, bypassCache = true,
            assignedAgentId = uid, userOrdersOnly = true
        )
        allOrders.clear()
    ...
    suspend fun loadNext(uid: String?): PageBundle? {
        ...
        val nextPage = page + 1
        val res = getMyOrders(
            page = nextPage, limit = pageSize, bypassCache = true,
            assignedAgentId = uid, userOrdersOnly = true
        )
        val next = res.items
...
fun withDistances(
    ...
    return if (location != null) computeDistancesUseCase(location, list) else list
}