...
class UpdateOrderStatusViewModel(
    ...
    // Which orders are currently updating (to disable buttons/spinner per card)
    ...
    private val _error = MutableSharedFlow<Pair<String, () -> Unit>>() // message + retry action
    ...
    fun update(orderId: String, targetStatus: OrderStatus, assignedAgentId: String? = null) {
        ...
        viewModelScope.launch {
            try {
                OrderLogger.postStart(orderId, targetStatus)              // ← start
                val serverOrder = updateStatus(orderId, targetStatus.toApiId(), assignedAgentId)
                OrderLogger.postSuccess(orderId, serverOrder.status)      // ← success
                _success.emit(serverOrder)
            } catch (t: Throwable) {
                OrderLogger.postError(orderId, targetStatus, t)           // ← error
                _error.emit((t.message ?: "Failed to update status") to { update(orderId, targetStatus, assignedAgentId) })
            } finally {
                _updatingIds.update { it - orderId }
            }
        }
